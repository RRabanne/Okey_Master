...
for color in COLORS:
    ...
    for i in range(1, 9):
        ...
        with row[i - 1]:
            st.markdown(
                f"<div onclick="window.location.href='?add={card}'">"
                f"<button style='width: 100%; padding: 0.5em; background-color: {background}; "
                f"border: 1px solid #666; border-radius: 8px;'>"
                f"<strong style='color: black;'>{i}</strong>"
                f"</button></div>",
...
for i in range(5):
    if i < len(st.session_state.active_hand):
        ...
        with active_cols[i]:
            st.markdown(
                f"<div style='background-color:{btn_color}; text-align:center; padding:0.5em; "
                f"border-radius:10px; font-size:24px; font-weight:bold;'>"
                f"<a href='?remove={card}' style='color:black; text-decoration:none'>{num}</a>"
                f"</div>",
                unsafe_allow_html=True
            )
...
if len(st.session_state.active_hand) == 5:
    ...
    st.markdown(
        f"<div style='background-color:{color_map[col]}; text-align:center; padding:0.5em; "
        f"border-radius:10px; font-size:24px; font-weight:bold;'>{num}</div>",
        unsafe_allow_html=True
    )
