...
for color in COLORS:
    ...
    for i in range(1, 9):
        ...
        with row[i - 1]:
            if st.button(f"{i}", key=f"{card}", help="Karte wählen", args=None):
                if not used and card not in st.session_state.active_hand and len(st.session_state.active_hand) < 5:
...
for i in range(5):
    if i < len(st.session_state.active_hand):
        ...
        with active_cols[i]:
            if st.button(f"{num}", key=f"remove_{card}", help="Karte ablegen"):
