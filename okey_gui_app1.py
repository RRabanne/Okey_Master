...
for color in COLORS:
    ...
    for i in range(1, 9):
        ...
        """
        with row[i - 1]:
            if st.markdown(f"<div onclick=\"window.location.href='?add={card}'\">{button_html}</div>", unsafe_allow_html=True):
                pass
...
if 'remove' in query_params:
    ...
    if card_to_remove in st.session_state.active_hand:
        ...
        st.session_state.removed.append(card_to_remove)  # Als abgeworfen markieren
    st.experimental_set_query_params()
